...
import { sanitizeHtml, sanitizeTextLinks } from 'email-sanitizer';
...
export default {
	...
	async email(message: EmailMessage, env: Env, ctx: ExecutionContext): Promise<void> {
		// 1. Initialize the core library for alias verification.
		const emailAliasCore = new EmailAliasCore({
			secretKey: env.SECRET_KEY,
			domain: env.DOMAIN,
		});

		// 2. Verify the HMAC signature of the recipient email alias.
		...
		const isSignatureValid = await emailAliasCore.isSignatureValid(message.to);
		if (!isSignatureValid) {
			message.setReject('Invalid recipient address. The alias signature is not valid.');
		...
		try {
			...
			const sanitizedText = email.text ? sanitizeTextLinks(email.text) : undefined;
			...
			const response = await fetch('https://api.mailchannels.net/tx/v1/send', {
				...
				body: JSON.stringify({
					...
					content: [
						...(sanitizedText ? [{ type: 'text/plain', value: sanitizedText }] : []),
						...(sanitizedHtml ? [{ type: 'text/html', value: sanitizedHtml }] : []),
					],
				}),
			});
